    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, MdfError> {
        ...
        if self.header.id != "##TX" {
            return Err(MdfError::BlockSerializationError(
                format!("TextBlock must have ID '##TX', found '{}'", self.header.id)
            ));
        }
        ...
        if self.header.block_len as usize != total_size {
            return Err(MdfError::BlockSerializationError(
                format!("TextBlock header.block_len ({}) does not match calculated size ({})", 
        ...
        if buffer.len() != total_size {
            return Err(MdfError::BlockSerializationError(
                format!("TextBlock has incorrect final size: expected {}, got {}", 
