...
use crate::mf4::blocks::metadata_block::MetadataBlock;
...
impl BlockHeader {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, MdfError> {
        ...
        if buffer.len() != 24 {
            return Err(MdfError::BlockSerializationError(
                format!("BlockHeader must be exactly 24 bytes, got {}", buffer.len())
            ));
        }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, MdfError> {
        ...
        Ok(Self {
            ...
            reserved0: LittleEndian::read_u32(&bytes[4..8]),
            block_len: LittleEndian::read_u64(&bytes[8..16]),
            links_nr: LittleEndian::read_u64(&bytes[16..24]),
        })
    }
}
